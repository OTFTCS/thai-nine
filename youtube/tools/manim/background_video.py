"""Generate per-block image videos from AI-generated images.

Reads block timestamps and imageRef from script JSON (timestamps are
written directly into lines by timestamp_audio.py).

Generates a left-zone video (1312x1080 by default) with FFmpeg xfade
transitions between images at block boundaries. Used as the background
for the left side of the split-frame YouTube layout.
"""
...
def generate_background_video(
    ...
    *,
    ...
    target_width: int = 1312,
    target_height: int = 1080,
) -> Path | None:
    """Build a left-zone image video from per-block images.
    ...
    _ffmpeg_concat(segments, output_path, crossfade_sec, target_width, target_height)
    return output_path
...
def _ffmpeg_concat(
    ...
    """Generate image video with xfade transitions between images."""
    if len(segments) == 0:
        return
    ...
    for i in range(len(segments)):
        filter_parts.append(
            ...
            f"crop={target_width}:{target_height},setsar=1[s{i}]"
        )
