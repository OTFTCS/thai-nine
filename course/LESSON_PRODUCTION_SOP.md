...
- Stage 1 also emits `script-spoken.html` as the readable browser/print companion.
...
- Stage 3 generates `deck-source.json`.
- Stage 3 generates `deck.pptx`.
- Stage 3 generates `asset-provenance.json`.
- Stage 3 generates `canva-content.json`.
- Stage 3 generates `canva-deck.pptx`.
- Stage 3 generates `canva-import-guide.md`.
- Stage 3 generates `canva-backgrounds/slide-XX.png`.
...
- Open the generated `deck.pptx` in PowerPoint or Keynote.
- Use the Canva pack with the master template if the recording deck will be finished in Canva.
- Keep `Sarabun` for Thai, transliteration, and English in Canva so mixed learner-facing lines stay in one readable family.
- Keep visible Thai in the learner-facing deck as `Thai (PTM transliteration)` on early lessons unless a later lesson has an approved reason to reduce support.
- Keep recording instructions in the import guide or ops docs, not in visible slide copy.
- Treat Canva edits as delivery-only unless you also back-port the layout fix into the repo contract/template.
- Record Nine using the PPTX deck in presenter mode.
